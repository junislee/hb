...
class FundsRateArb(ScriptStrategyBase):


    spot_connector = "binance"
    perp_connector = "binance_perpetual"
    ...
    def get_fund_rate(self):
        funding_info = self.connectors["binance_perpetual"].get_funding_info(self.trading_pair)
        self.funding_rate = funding_info.rate
        self.logger().info(funding_info.rate)
